...
from planning_utils import a_star, heuristic, create_grid, create_grid_and_edges, prune_path, create_graph, a_star_graph, create_graph_from_edges
from udacidrone import Drone
from udacidrone.connection import MavlinkConnection
from udacidrone.messaging import MsgID
from udacidrone.frame_utils import global_to_local

from sampler_utils import Sampler
...
class PointsOfInterest():
    def __init__(self):
        self.poi_dict = {
        "Mission Spear" : (float(-122.39314), float(37.79270), 0.0 ),
        "Washington Street" : (float(-122.40131), float(37.79666), 0.0 ),
        "California Street" : (float(-122.39624), float(37.79391), 0.0 ),
        "Park" :  (float(-122.39374), float(37.79575), 0.0 ),
        "Sacramento Front" : (float(-122.39931), float(37.79471), 0.0 )
        }
...
class MotionPlanning(Drone):
    ...
    def __init__(self, connection):
        ...
        self.grid, self.north_offset, self.east_offset, self.edges = create_grid_and_edges(data, TARGET_ALTITUDE, SAFETY_DISTANCE)
    ...
    def plan_path(self):
        ...
        # goal = global_to_local(self.poi.get("California Street"), self.global_home)
        # start = (start_pos[0], start_pos[1], start_pos[2])
        # goal = (goal[0], goal[1], goal[2])
        # sampler = Sampler(data)
        # polygons = sampler._polygons
        # nodes = sampler.sample(600, start, goal)
        # print("Number of nodes: ",len(nodes))
        # g = create_graph(nodes, 10, polygons)
        # path, _ = a_star_graph(g, heuristic, start_pos, goal)
        ...
        goal = global_to_local(self.poi.get("Sacramento Front"), self.global_home)
        ...
        g_start = tuple(nodes[node_start])
        g_goal =  tuple(nodes[node_goal])
